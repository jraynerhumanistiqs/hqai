...
export function BulkActionFooter({ selectedCount, selectedIds, onCompare, onClear, onAfterBulk }: Props) {
  ...
  return (
    <>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-ink text-bg rounded-full shadow-card px-4 py-2 flex items-center gap-3">
        ...
        <div ref={stageRef} className="relative">
          <button
            ...
            disabled={busy}
            className="text-xs font-bold px-3 py-1 rounded-full bg-bg/10 text-bg hover:bg-bg/20 disabled:opacity-40"
        ...
        <button
          onClick={bulkReject}
          disabled={busy}
          className="text-xs font-bold px-3 py-1 rounded-full bg-danger/20 text-bg hover:bg-danger/30 disabled:opacity-40"
        >Reject all</button>

        <button
          onClick={onClear}
          className="text-xs font-bold text-bg/70 hover:text-bg"
        >Clear</button>
      </div>

      {toast && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-ink text-bg rounded-2xl shadow-card px-4 py-3 text-xs font-bold max-w-md">
          {toast}
        </div>
      )}
    </>
