  ...
  return (
    <div className="h-full overflow-y-auto scrollbar-thin bg-[#000000]">
      <div className="max-w-3xl mx-auto px-8 py-8">
        <h1 className="font-display text-h1 font-bold text-white uppercase tracking-wide mb-1">HR Templates</h1>
        <p className="text-sm text-gray-400 mb-8">Best practice templates curated by HQ.ai experts. Download unmarked or customise with your business details first.</p>
            ...
            <div key={cat.title} className="bg-[#111111] border border-[#222222] rounded-2xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  <h2 className="font-display text-lg font-bold text-white uppercase tracking-wider">{cat.title}</h2>
                  <span className="text-xs text-gray-500 bg-[#1a1a1a] px-2 py-0.5 rounded-full">{cat.templates.length}</span>
                </div>
              ...
              {openCategory === cat.title && (
                <div className="border-t border-[#222222]">
                  {cat.templates.map((tmpl, idx) => (
                    <div key={tmpl.name}
                      className={`px-6 py-4 flex items-start gap-4 hover:bg-[#0a0a0a] transition-colors ${idx > 0 ? 'border-t border-[#1a1a1a]' : ''}`}>
                      <div className="w-8 h-8 bg-[#fd7325]/15 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <div className="relative group">
                          <button className="bg-[#1a1a1a] hover:bg-[#222222] text-gray-400 hover:text-white text-xs font-bold px-3 py-2 rounded-lg border border-[#333333] transition-colors">
                            Download
                          </button>
                          <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#222222] text-white text-[10px] px-2.5 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-lg">
                            Download unmarked template
                          </span>
                        </div>
                        <div className="relative group">
                          <button className="bg-[#fd7325] hover:bg-[#e5671f] text-white text-xs font-bold px-3 py-2 rounded-lg transition-colors">
                            Customise
                          </button>
                          <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#222222] text-white text-[10px] px-2.5 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-lg">
