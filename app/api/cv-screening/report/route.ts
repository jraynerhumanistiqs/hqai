...
export async function POST(req: NextRequest) {
  try {
    ...
    const buildCandidateDocx = async (s: CandidateScreening): Promise<Buffer> => {
    ...
    // Single candidate -> return one Word doc (legacy behaviour preserved).
    ...
    // Multiple candidates -> zip of individual DOCX files, named after the
    // role title. All selected candidates SHOULD share one rubric (filtered
    // in the UI). If they happen to span rubrics, name the zip "Mixed roles".
    const rubricRoles = new Set<string>()
    for (const s of screenings) {
      const r = await resolveRubric(s.rubric_id)
    ...
    // Generate docs concurrently to keep p95 under the 60s function ceiling
