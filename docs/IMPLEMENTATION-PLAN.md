...
| A4 | Colour-literal audit | done | Bulk swap landed: 220 `bg-white` -> `bg-bg-elevated`, 16 `focus:border-black` -> `focus:border-ink`, and `text-black` -> `text-ink` across 29 dashboard / prescreen / review / chat / recruit / cv-screening files. Opacity variants like `bg-white/10` (used as highlights inside the dark sidebar) were preserved via word-boundary regex. `bg-black` deliberately left alone because it serves both as a CTA colour and as dot/indicator decoration that should stay black in either mode. |
